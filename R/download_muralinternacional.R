...
#'
#' @examples
download_muralinternacional <- function(
  ...
  url_archive <- "https://www.e-publicacoes.uerj.br/index.php/muralinternacional/issue/archive"
  ...
  if(isTRUE(info_data)){
    ...
    dat <- purrr::imap_dfr(pdfs$url, function(x, .y) {
      ...
      loc_arquivo <- paste0(
        ...
        pdfs$vol[.y],
        "-",
        if(is.na(pdfs$n[.y])) "n.1" else pdfs$n[.y],
        "-",
    ...
    purrr::imap(pdfs$url, function(x, .y) {
      ...
      path_file <- paste0(dir,"/", pdfs$ano[.y], "-", pdfs$vol[.y],"-",if(is.na(pdfs$n[.y])) "n.1" else pdfs$n[.y],"-",ifelse(.y < 10, paste0("0",.y) , .y),".pdf")
