...
#'
#' @examples
download_bjlas <-  function(year, volume, number, dir,  info_data = FALSE) {
  ...
  xml2::read_html(url_archive2) %>%
   ...
   for(i in seq_along(eds)){
     if(!stringr::str_detect(eds[i], "v\\.")){
       xml2::read_html(primary_url[i]) %>%
         rvest::html_nodes('h1') %>%
         rvest::html_text() %>%
         stringr::str_remove_all("\\n|\\t") %>%
         stringr::str_remove_all(":(.*)") -> ed_certa
